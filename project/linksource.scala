...
object LinkSourcePlugin extends AutoPlugin {
  ...
  object autoImport {
    lazy val javadocSourceBaseUrl =
      settingKey[Option[String]]("base URL (no trailing slash) for source code")
  }
  ...
  private def rewriteSourceLinks(dir: File,
    ...
    for (f <- htmlFiles) {
      ...
      val changed = origRegex.replaceAllIn(
        content, { m: scala.util.matching.Regex.Match =>
          val oldFileLine = m.group(1)
          val fileLine =
            oldFileLine.replace("line.", "L").replace(".html", ".java")
