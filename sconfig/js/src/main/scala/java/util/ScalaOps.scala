...
private[util] object ScalaOps {
  ...
  private class JavaIteratorAdapter[A](
      scalaIterator: scala.collection.Iterator[A])
      extends Iterator[A] {
    def hasNext(): Boolean = scalaIterator.hasNext
    def next(): A          = scalaIterator.next()

    override def remove(): Unit = // *** added override for 2.12 ***
  ...
  class JavaIteratorOps[A] private[ScalaOps] (val __self: Iterator[A])
    ...
    @inline def foreach[U](f: A => U): Unit = {
      while (__self.hasNext()) f(__self.next())
    }
    ...
    @inline def foldLeft[B](z: B)(f: (B, A) => B): B = {
      var result: B = z
      while (__self.hasNext()) result = f(result, __self.next())
      result
    }
  ...
  private class MappedIterator[A, B](iter: Iterator[A], f: A => B)
    ...
    override def remove(): Unit =
      iter.remove() // *** added override for 2.12 ***
  }

  @inline
  final class IteratorWithFilter[A] private[ScalaOps] (iter: Iterator[A],
                                                       pred: A => Boolean) {
  ...
  class JavaEnumerationOps[A] private[ScalaOps] (val __self: Enumeration[A])
    ...
    @inline def foreach[U](f: A => U): Unit = {
      while (__self.hasMoreElements()) f(__self.nextElement())
    }
  }
