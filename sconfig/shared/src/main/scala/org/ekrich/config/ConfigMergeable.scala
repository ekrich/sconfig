 ...
 * [[ConfigValue]]. Instances of `Config` and `ConfigValue` can be combined into
 * a single new instance using the
 * [[ConfigMergeable#withFallback withFallback()]] method.
 *
 * ''Do not implement this interface''; it should only be implemented by the
...
trait ConfigMergeable {
  ...
  /**
   * Returns a new value computed by merging this value with another, with keys
   * in this value "winning" over the other one.
   *
   ...
   * Merging typically occurs when either the same object is created twice in
   * the same file, or two config files are both loaded. For example:
   *
   * {{{
   *   foo = { a: 42 }
   *   foo = { b: 43 }
   ...
   * }}}
   *
   * Here, the number 10 "wins" and the value of `foo` would be simply 10.
   * Again, for details see the spec.
   *
   * @param other
   *   an object whose keys should be used as fallbacks, if the keys are not
   *   present in this one
   * @return
