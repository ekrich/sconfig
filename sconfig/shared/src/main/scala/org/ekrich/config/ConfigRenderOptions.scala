...
/**
 * <p> A set of options related to rendering a {@link ConfigValue}. Passed to
 * [[ConfigValue!.render(options:org\.ekrich\.config\.ConfigRenderOptions)* ConfigValue.render(ConfigRenderOptions)]].
 *
 * <p> Here is an example of creating a {@code ConfigRenderOptions}:
 *
...
object ConfigRenderOptions {
   ...
   * {@link Config}, the concise rendering will be valid JSON.
   *
   * @return
...
final class ConfigRenderOptions private (
   ...
   * controlled by {@link ConfigRenderOptions#setOriginComments}.
   *
   * @param value
   ...
   * library generates comments for each setting based on the {@link
   * ConfigValue#origin} of that setting's value. For example these comments
   * might tell you which file a setting comes from.
   *
   * <p> {@code setOriginComments(Boolean)} controls only these autogenerated
   * "origin of this setting" comments, to toggle regular comments use {@link
   ...
   * use comments is controlled by the separate {@link #setComments} and {@link
   * #setOriginComments} options. So if you enable comments you will get invalid
   * JSON despite setting this to true.
   *
   * @param value
