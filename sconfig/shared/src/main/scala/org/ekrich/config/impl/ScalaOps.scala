// Ported from Scala.js commit: d6d6a37 dated: 2025-08-11
...
private[config] object ScalaOps {
...
//   }
  ...
  implicit class ToJavaIteratorOps[A] private[ScalaOps] (
      val __self: ju.Iterator[A]
  ) extends AnyVal {
    def scalaOps: JavaIteratorOps[A] = new JavaIteratorOps[A](__self)
  }

  class JavaIteratorOps[A] private[ScalaOps] (val __self: ju.Iterator[A])
      extends AnyVal {
     ...
     * java.lang.StringBuilder for Scala Native / JVM
     */
    @inline def mkString(start: String, sep: String, end: String): String = {
  ...
  implicit class ToJavaEnumerationOps[A] private[ScalaOps] (
      val __self: ju.Enumeration[A]
  ) extends AnyVal {
    def scalaOps: JavaEnumerationOps[A] = new JavaEnumerationOps[A](__self)
  }

  class JavaEnumerationOps[A] private[ScalaOps] (val __self: ju.Enumeration[A])
      extends AnyVal {
